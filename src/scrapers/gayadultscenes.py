...
class GayAdultScenesScraper(BaseScraper):
    def __init__(self, http_client: httpx.AsyncClient) -> None:
        self._client = http_client

    @property
    def source_key(self) -> str:
        return "gayadultscenes"

    @property
    def source_name(self) -> str:
        return "Gay Adult Scenes"
    ...
    async def search(self, title: str, year: int | None = None) -> list[MetadataItem]:
        ...
        for source_key in SCENE_SOURCES:
            ...
            try:
                ...
                logger.exception("GayAdultScenes delegate search failed for %s", source_key)
                continue
...
def _split_delegate_source(source_id: str) -> tuple[str, str]:
    ...
    if not sep or not source_key or not delegate_id:
        raise ValueError(f"Invalid GayAdultScenes source id: {source_id}")
