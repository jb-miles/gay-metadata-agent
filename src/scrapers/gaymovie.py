...
class GayMovieScraper(BaseScraper):
    ...
    async def search(self, title: str, year: int | None = None) -> list[MetadataItem]:
        ...
        for _page in range(MAX_PAGES):
            ...
            try:
                ...
                logger.exception("GayMovie search failed for %s", current_url)
                break

            tree = lxml_html.fromstring(response.text)

            film_nodes = tree.xpath('//div[@class="fusion-post-wrapper"]')
            if not film_nodes:
                break
            ...
            for node in film_nodes:
                ...
                film_title = _first_text(node.xpath('.//div/div/h2/a/text()'))
                if not film_title:
                    # Fallback: try h2/a directly
                    film_title = _first_text(node.xpath('.//h2/a/text()'))
                if not film_title:
                    continue

                href = _first_text(node.xpath('.//div/div/h2/a/@href'))
                if not href:
                    href = _first_text(node.xpath('.//h2/a/@href'))
                if not href:
                    continue
        ...
        logger.info("GayMovie search for %r returned %d results", title, len(results))
        return results
    ...
    async def get_metadata(self, source_id: str) -> MetadataItem:
        ...
        if not studio:
            studio = _first_text(tree.xpath('//strong[contains(.,"Studio:")]/following-sibling::a/text()'))
        if not studio:
            studio_text = _first_text(
                tree.xpath('//strong[contains(.,"Studio:")]/following-sibling::text()')
            )
            if studio_text:
                studio = studio_text.strip().strip(":").strip() or None

        # Cast
        cast: list[str] = []
        cast_links = tree.xpath(
            '//strong[contains(.,"Actors") or contains(.,"Cast")]/following-sibling::a/text()'
        )
        if cast_links:
        ...
        raw_genres = [
            ...
            if g.lower() not in {"movies", "home", "uncategorized", "gay movie"}
        ]
